...
const PostDetail = () => {
    ...
    useEffect(() => {
        const fetchPost = async () => {
            try {
                ...
                if (currentUser) {
                    const likeStatusRes = await axios.get(`${API_BASE_URL}/api/posts/${id}/likes/${currentUser.id}`);

                    setLiked(likeStatusRes.data.liked);
                }

                const likeCountRes = await axios.get(`${API_BASE_URL}/api/posts/${id}/likes/count`);

                setLikeCount(likeCountRes.data.count);

                // Fetch comments
                const commentsRes = await axios.get(`${API_BASE_URL}/api/posts/${id}/comments`);

                setComments(commentsRes.data.comments);
                setCommentCount(commentsRes.data.totalCount);
    ...
    const handleLike = async () => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.error || error.response?.data?.details || 'Failed to update like';
    ...
    const handleAddComment = async (e, parentId = null) => {
        ...
        try {
            await axios.post(`${API_BASE_URL}/api/posts/${id}/comment`, {
            ...
            const errorMessage = error.response?.data?.error || error.response?.data?.details || 'Failed to add comment';
            toast.error(errorMessage);
        } finally {
    ...
    const handleDeleteComment = async (commentId) => {
        ...
        try {
            await axios.delete(`${API_BASE_URL}/api/posts/${id}/comment/${commentId}`, {
    ...
    const handleLikeComment = async (commentId) => {
        ...
        try {
            await axios.post(`${API_BASE_URL}/api/posts/${id}/comment/${commentId}/like`, {
                    ...
                    <div className="flex items-center gap-4 mb-8">
                        <Link to={`/profile/${post.author?.id}`}>
                            <img
                                ...
                                className="w-14 h-14 rounded-full object-cover border-2 border-slate-100 dark:border-slate-800 hover:opacity-80 transition-opacity"
                            />
                        </Link>
                        <div>
                            <Link to={`/profile/${post.author?.id}`} className="text-xl font-bold text-slate-900 dark:text-white hover:text-blue-600 transition-colors">
                                {post.author?.name}
                            </Link>
                                ...
                                <div className="space-y-8">
                                    {comments.length === 0 ? (
                                        ...
                                        (() => {
                                            ...
                                            // Sort top-level by date desc, but replies by date asc
                                            tree.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
                                            tree.forEach(t => t.replies.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)));
                                                        ...
                                                        <div className="group flex gap-4">
                                                            <Link to={`/profile/${comment.user?.id}`}>
                                                                <img
                                                                    ...
                                                                    className="w-10 h-10 rounded-full object-cover border-2 border-slate-100 dark:border-slate-800 shrink-0 hover:opacity-80 transition-opacity"
                                                                />
                                                            </Link>
                                                            <div className="flex-1 min-w-0">
                                                                <div className="flex items-center gap-2 mb-1">
                                                                    <Link to={`/profile/${comment.user?.id}`} className="font-bold text-slate-900 dark:text-white text-sm truncate hover:text-blue-600 transition-colors">
                                                                        {comment.user?.name}
                                                                    </Link>
                                                                ...
                                                                {replyingTo === comment.id && (
                                                                    <div className="mt-4 flex gap-3 animate-in fade-in slide-in-from-top-2">
                                                                        <img src={currentUser.image || authorPlaceholder} className="w-8 h-8 rounded-full object-cover" alt="" />
                                                                        <div className="flex-1">
                                                                            <textarea
                                                                                value={replyContent}
                                                                                onChange={(e) => setReplyContent(e.target.value)}
                                                                                placeholder="Write a reply..."
                                                                                className="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                                                                                rows="2"
                                                                            />
                                                                            <div className="flex justify-end mt-2">
                                                                                <button
                                                                                    onClick={() => handleAddComment(null, comment.id)}
                                                                                    disabled={submittingComment || !replyContent.trim()}
                                                                                    className="px-4 py-1.5 bg-blue-600 text-white rounded-lg font-bold text-xs disabled:opacity-50"
                                                                                >
                                                                ...
                                                                {comment.replies && comment.replies.length > 0 && (
                                                                    <div className="mt-6 space-y-4 ml-4 border-l-2 border-slate-100 dark:border-slate-800 pl-4">
                                                                        {comment.replies.map((reply) => (
                                                                            <div key={reply.id} className="flex gap-3">
                                                                                <Link to={`/profile/${reply.user?.id}`}>
                                                                                    <img
                                                                                        src={reply.user?.image || authorPlaceholder}
                                                                                ...
                                                                                <div className="flex-1 min-w-0">
                                                                                    <div className="flex items-center gap-2 mb-0.5">
                                                                                        <Link to={`/profile/${reply.user?.id}`} className="font-bold text-slate-900 dark:text-white text-xs hover:text-blue-600">
                                                                                            {reply.user?.name}
                                                                                        </Link>
                                                                                        {post.authorId === reply.userId && (
                                                                                            <span className="text-white bg-blue-600 px-1.5 py-0.5 rounded-[4px] text-[8px] font-black uppercase">Author</span>
                                                                                        )}
